		...
		for (int x = Math.max(0, i - 1); x <= Math.min(StaticTool.allrow - 1,
			...
			for (int y = Math.max(0, j - 1); y <= Math.min(
				...
				if (mineLabel[x][y].isFlagTag() == false
						&& mineLabel[x][y].isExpendTag() == false) {
		...
		if (needCount == expendCount) {
			...
			if (level != 0) {
				if (level == 1) {
					...
					if (name != null) {
						StaticTool.treeSetC.add(new Leader(
					...
					String name = JOptionPane.showInputDialog(mainFrame,
							"Please Enter Your Name: ");
					if (name != null) {
						StaticTool.treeSetZ.add(new Leader(
								StaticTool.timecount, name));
					}
				} else if (level == 3) {
					String name = JOptionPane.showInputDialog(mainFrame,
							"Please Enter Your Name: ");
					if (name != null) {
						StaticTool.treeSetG.add(new Leader(
								StaticTool.timecount, name));
					}
				}
